'use strict';

var _telegramJs = require('telegram-js');

var _telegramMtNode = require('telegram-mt-node');

var _telegramMtNode2 = _interopRequireDefault(_telegramMtNode);

var _telegramTlNode = require('telegram-tl-node');

var _telegramTlNode2 = _interopRequireDefault(_telegramTlNode);

var _apiSchema = require('./api-schema.json');

var _apiSchema2 = _interopRequireDefault(_apiSchema);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

// get server IPs and your app id/hash from http://my.telegram.org/
var APP_ID = '123123';

// source: https://github.com/dot-build/telegram-js/blob/master/sample.js

// Download from https://core.telegram.org/schema/json
// Make sure you have the most recent layer version!

var APP_HASH = '123123';
var SERVER = { host: '149.154.167.40', port: '443' };

// Factory instance, where new clients will be generated
var telegram = new _telegramJs.Telegram(_telegramMtNode2.default, _telegramTlNode2.default);
telegram.useSchema(_apiSchema2.default);
addPublicKeys(telegram);

var config = {
    id: APP_ID,
    hash: APP_HASH,
    version: '0.0.1',
    lang_code: 'en',
    authKey: null
};

// on Node.js side, is better to use TcpConnection instead
var connection = new _telegramMtNode2.default.net.HttpConnection(SERVER);

// You can create as many clients as necessary. Usually, you will have two
// instances, one for API calls and one for file transport
var client = telegram.createClient();
client.setConnection(connection);

connection.connect(function () {
    var ready = client.setup(config);

    ready.then(function (client) {
        function callback(response) {
            console.log(response);
        }

        client.callApi('help.getConfig').then(callback, callback);
    });
});

// Telegram public keys
function addPublicKeys(telegram) {
    var publicKeys = [{
        fingerprint: '0x9a996a1db11c729b',
        modulus: '',
        exponent: '010001'
    }, {
        fingerprint: '0xb05b2a6f70cdea78',
        key: '',
        exponent: '010001'
    }, {
        fingerprint: '0xc3b42b026ce86b21',
        modulus: '',
        exponent: '010001'
    }, {
        fingerprint: '0x71e025b6c76033e3',
        modulus: '',
        exponent: '010001'
    }];

    publicKeys.forEach(function (key) {
        return telegram.addPublicKey(key);
    });
}