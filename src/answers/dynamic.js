...
const smartAnswerExample = (state, name, votes, year) => {
  ...
  const bothShifts = [
    `${name} teve em ${state} ${votes.first} votos no primeiro turno e ${votes.second} votos no segundo turno, totalizando ${votes.total} se somado os dois turnos`,
    `${name} recebeu em ${state} ${votes.first} votos no primeiro turno e ${votes.second} votos no segundo turno. Tendo um total de votos de ${votes.total} somando os dois turnos`,
    ...
    `${votes.total} votos no total, sendo ${votes.first} no primeiro turno e ${votes.second} no segundo.`,
...
exports.topVotingStateMessages = (result, year, name) => {
  ...
  const success = `O estado que mais votou em ${name} no ano de ${year} foi ${state} com um total de ${votes.total} votos (somando os dois turnos).`
  return result ? success : failure
}
...
exports.mostVotedInYearByStateMessages = (result, year, state) => {
  ...
  const success = `O candidato mais votado de ${state} em ${year} foi ${name} com ${votes.total} votos, somando os votos dos dois turnos.`
  return result ? success : failure
}
